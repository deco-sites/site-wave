...
const JobList = ({ jobs, filterOptions }: Props) => {
  const [selectedWorkModel, setSelectedWorkModel] = useState<string | undefined>();
  ...
  const areaOptions = ["Designer", "Projetos", "SEO", "Tráfego Pago", "Desenvolvimento"];
  ...
  const filteredJobs = jobs.filter((job) => {
    const workModelMatch = selectedWorkModel ? job.workModel === selectedWorkModel : true;
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (workModelRef.current && !workModelRef.current.contains(event.target as Node)) {
      ...
      <div class="lg:w-3/4 lg:ml-10 flex flex-col gap-3">
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => (
            <div class="container flex justify-between items-center py-8 px-4 lg:px-8 bg-[#0f0f0f] rounded-3xl" key={job.title}>
              <div>
                <p class="text-white text-base font-extrabold">{job.title}</p>
                <p class="text-[#0066E4] text-sm font-light mt-3">{job.workModel}</p>
              </div>
              <div>
