...
const CardBrand = ({ brand, services, settings }: Props) => {
    ...
    return (
        <div class={`container flex ${getAlignment(settings?.alignment)} ${settings?.sticky && 'sticky top-[110px]'} `}>
            <div
                class={`rounded-[20px] h-fit flex flex-col gap-[25px] w-full 
                        ${settings?.hasBg ? "bg-[#0A0A0A] px-10 min-w-[358px]" : "lg:max-w-[666px]"} ${settings?.hasLogo ? "py-10" : "pb-10"}`}
            >
