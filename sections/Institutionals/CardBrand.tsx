...
const CardBrand = ({ brand, services, settings }: Props) => {
  ...
  return (
    <div
      class={`container flex ${getAlignment(settings?.alignment)} ${
        settings?.sticky && "sticky top-[110px]"
      } `}
    >
      <div
        class={`rounded-[20px] h-fit flex flex-col gap-[25px] w-full 
                        ${
          settings?.hasBg ? "bg-[#0A0A0A] px-10 min-w-[358px]" : ""
        } ${settings?.hasLogo ? "py-10" : "pb-10"}`}
      >
        <div class={`${settings?.hasLogo && "pb-5 border-b border-[#393939]"}`}>
          {settings?.hasLogo && brand?.logo && (
            <img width={175} height={61} src={brand.logo} alt={brand.alt} />
          )}
        </div>
        {services?.map((service) => {
          ...
          return (
            <div class="flex flex-row items-start gap-[10px]">
              <div
                class={`bg-black rounded-[10px] w-10 h-auto ${
                  settings?.hasLogo && "p-[15px]"
              ...
              <div class="flex items-center">
                <p class="uppercase text-base text-[#717171]">
                  {service?.title && (
                    <span style={{ color }} class="mr-[5px] uppercase">
                      {service.title}
                    </span>
                  )}
                  {service?.paragraph}
