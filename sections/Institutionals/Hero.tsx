...
interface Props {
    ...
    isMobile: boolean;
    selectorType: 'services' | 'partners';
}

export const loader = (props: Props, req: Request, ctx: AppContext) => {
    const { servicesSelector: options = [], partnerSelector: partner = [], device } = ctx;
    const isMobile = device === "mobile";
    const selectedOptions = props.selectorType === 'services' ? options : partner;

    return { ...props, options: selectedOptions, isMobile };
};

export default function Hero({
    bgImage,
    title = "Sua marca em evidência.",
    subtitle = "Potencialize os resultados de vendas e aumente o faturamento do seu e-commerce com um time de profissionais qualificados e certificados nas maiores plataformas de mídia paga.",
    cta,
    labelBottom = { text: "", titulo: undefined },
    options,
    isMobile,
    findOutMore,
    selectorType,
    ...
    return (
        <div class="bg-cover bg-center bg-no-repeat relative flex items-center justify-start" style={{
            height: `${isMobile ? (mobileHeight === 0 ? desktopHeight : mobileHeight) : desktopHeight}px`,
            ...
            <div class="container px-5 lg:p-0 relative z-20 lg:w-full sm:w-2/3 m-auto">
                ...
                {labelBottom && (
                    <div class="flex flex-col items-start gap-3">
                        {labelBottom.titulo && (
                            <p class="font-bold text-sm uppercase text-blue-700 max-w-[148px]">
                                {labelBottom.titulo}
                            </p>
                        )}
