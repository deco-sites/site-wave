...
const getLayoutClasses = (
  layoutType: "grid" | "flex",
  columnLayout: "primary",
): string => {
  if (layoutType === "grid") {
    const gridClasses = {
      primary:
        "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 items-center",
    };
...
export default function PageLayout({ sections, layoutSettings }: Props) {
  return (
    <div
      class={`container justify-center px-5 lg:px-0 ${
        getLayoutClasses(layoutSettings.layoutType, layoutSettings.columnLayout)
      }`}
    >
