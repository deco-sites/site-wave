...
export default function Hero({ bgImage, labelTop = {
    text: "",
}, title = "Sua marca em evidência.", subtitle = "Potencialize os resultados de vendas e aumente o faturamento do seu e-commerce com um time de profissionais qualificados e certificados nas maiores plataformas de mídia paga.", cta, labelBottom = {
    text: "",
}, options, isMobile, }: SectionProps<typeof loader>) {
    ...
    return (<div class="bg-cover bg-center bg-no-repeat relative flex items-center justify-start" style={{
      ...
      <div class="container px-5 md:px-16 lg:px-36 py-10 relative z-20 w-full sm:w-2/3 lg:w-1/2 ml-0 mr-auto">
        ...
        {labelTop && (<div class="flex items-center gap-3 mt-10">
            {labelTop.icon && (<Icon id={labelTop.icon} size={20} class="text-white"/>)}
        ...
        {cta && (<a class="block w-fit bg-[#0066E4] rounded-[30px] border-2 border-[#0066e4] text-white py-3 px-5 mb-10" href={cta.href}>
            <div dangerouslySetInnerHTML={{ __html: cta.text }}/>
          </a>)}
        {labelBottom && (<div class="flex items-center gap-3">
            {labelBottom.icon && (<Icon id={labelBottom.icon} size={16} class="text-[#717171]"/>)}
            {labelBottom?.text &&
