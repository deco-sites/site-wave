...
const CONTENT_STYLES =
  `w-full px-4 lg:px-0 ${PARAGRAPH_STYLES} ${HEADING_STYLES} ${CODE_BLOCK_STYLES} ${IMAGE_STYLES} ${BLOCKQUOTE_STYLES}`;
...
export default function BlogPost({ page }: Props) {
  const { title, authors, image, date, content, categories } = page?.post || DEFAULT_PROPS;
  ...
  return (
    <div class="w-full flex flex-col gap-20  px-4 md:px-0 pb-12 lg:pb-28 text-white mt-5">
      <div class=" w-full flex flex-col gap-12  lg:mx-auto" height={"460px"}>
        <div style={{
          background: `
            linear-gradient(270deg, rgba(0, 0, 0, 0.00) 0%, #000 100%), 
            linear-gradient(180deg, rgba(0, 0, 0, 0.00) 0%, #000 100%), 
            url(${device === "desktop" ? image : image}) lightgray 50% / cover no-repeat
          `,
          borderRadius: '16px',
          height: '460px'
        }} >
          <div class="container flex justify-start items-center h-full pb-10">
            <div class="lg:max-w-[700px] flex flex-col gap-7 h-full justify-end">

              <h1 class="text-[#717171] uppercase text-base font-bold">{categories?.[0]?.name}</h1>
              <h2 class="lg:text-5xl font-bold">{title}</h2>
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-row gap-2">
                  ...
                  <p class="text-[#717171] text-base">|</p>
                  <p class="text-[#717171] text-base">{calculateReadingTime(content.split(" ").length)}</p>
                  <p class="text-[#717171] text-base">|</p>
                  <p class="text-[#717171] text-base">{`  ${formattedDate}`}</p>
      ...
      <div class="container w-full">
        {/* <img
        class="object-cover rounded-2xl m-auto w-[auto]"
        src={image || ""}
      /> */}
        <div
        ...
        <div class="flex flex-col gap-10 w-full mx-auto mt-16 px-4 lg:px-0">
          {/* divider zinc-300 */}
          {/* <div class="w-full h-px bg-zinc-300"></div> */}
          <div class="flex items-center gap-4 bg-[#0A0A0A] rounded-xl p-10">
            <Image
              class="object-cover border border-[#393939]  rounded-full"
              alt={authors[0]?.name}
              src={authors[0]?.avatar || ""}
              width={145}
              height={145}
            ...
            <div class="flex flex-col">
              <p class="font-semibold text-base lg:text-2xl text-white">
                Escrito por {authors[0].name}
              </p>
