...
function Container({ children }: {
    ...
    return (<div class="container lg:mx-auto lg:py-14 mx-2 py-12 text-sm text-white">
    ...
    return (<ContainerComponent>
      <>
        <div class="gap-8 grid grid-cols-1 lg:grid-cols-4 md:grid-cols-3">
          {posts?.slice(from, to).map((post) => (<a href={`/blog/${post.slug}`} class="border border-[#0066E4] overflow-hidden rounded-lg">
              <Image width={380} height={274} class="object-fit w-full" sizes="(max-width: 640px) 100vw, 30vw" src={post.image || ""} alt={post.image} decoding="async" loading="lazy"/>
              <div class="p-6 space-y-4">
                ...
                <div class="flex flex-wrap gap-2">
                  {post.categories?.map((category) => (<div class="badge badge-lg badge-primary text-xs">
                      {category.name}
                    </div>))}
                </div>
                <div class="flex flex-wrap gap-2">
        ...
        {posts && to < posts.length && (<div class="flex justify-center w-full" id={postList}>
            <button hx-get={fetchMoreLink} hx-swap="outerHTML" hx-target={`#${postList}`} aria-label={cta.text} class="btn btn-primary">
